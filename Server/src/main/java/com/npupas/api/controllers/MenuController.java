...
public class MenuController {
	...
	public ResponseEntity<MessageDTO> createProduct(@RequestHeader("Authorization") String token, @Valid AddProductDTO product, BindingResult result){
		try {
			...
			return new ResponseEntity<MessageDTO>(new MessageDTO("Ocurrió un problema con la petición."), HttpStatus.BAD_REQUEST);
		} catch (Exception e) {
			System.out.println(e.getMessage());
			e.printStackTrace();
			return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
	...
	@GetMapping("/menu/{id}")
	public ResponseEntity<Product> getPupuseriaMenuProduct(@PathVariable("id") Long id, @RequestHeader("Authorization") String tokenStr){
		try {
			...
			else return new ResponseEntity<Product>(product, HttpStatus.OK);
		} catch (Exception e) {
			System.out.println(e.getMessage());
			return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
		}
	...
	public ResponseEntity<MessageDTO> putPupuseriaMenuProduct(@RequestHeader("Authorization") String tokenStr, @PathVariable("product_id") Long productId, @Valid AddProductDTO productDTO, BindingResult result ){
		try {
			if(result.hasErrors()) {
				return new ResponseEntity<MessageDTO>(new MessageDTO("Datos de la petición son incorrectos."), HttpStatus.BAD_REQUEST);
			}
			
			if(menuService.updateProduct(tokenStr, productId, productDTO)) return new ResponseEntity<MessageDTO>(new MessageDTO("Producto modificado con éxito."), HttpStatus.CREATED);
	...
	public ResponseEntity<MessageDTO> deletePupuseriaMenuProduct(@RequestHeader("Authorization") String tokenStr,  @PathVariable("product_id") Long productId){
		try {
			if(menuService.deleteProduct(tokenStr, productId)) return new ResponseEntity<MessageDTO>(new MessageDTO("Producto borrado con éxito."), HttpStatus.OK);
			else return new ResponseEntity<MessageDTO>(new MessageDTO("Petición mal realizada."), HttpStatus.BAD_REQUEST);
		} catch (Exception e) {
			System.out.println(e.getMessage());
